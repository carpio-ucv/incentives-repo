...
mr_data[[3]]=ordered(mr_data[[3]])#as.factor
...
pp.exp <- cbind(dexp, predict(m8, newdata = dexp, 
                               type = "probs", se = TRUE))
...
### PROBABILITY PLOT OF THE BEST PREDICTIVE MODEL (expectations separate)

```{r}
modexp2<- multinom(variable ~ r_type + tot_exp2 + income + 
                r_amount + r_amount*tot_exp2,data = mr_data)
...
dexp<-expand.grid(v.r_type, v.exp_all, v.r_amount, v.income)
colnames(dexp)<-c("r_type", "tot_exp2", "r_amount", "income")


## store the predicted probabilities for each value of ses and write
pp.exp <- cbind(dexp, predict(modexp2, newdata = dexp, 
                               type = "probs", se = TRUE))
...
## melt data set to long for ggplot2
lpp <- melt(pp.exp, id.vars = c("tot_exp2", "r_type",
                "income", "r_amount"), value.name = "probability")
...
ggplot(lpp2, aes(x = tot_exp2, y = probability, colour = r_type)) +
        geom_line(size=1) +
        facet_grid(variable ~ r_amount*income, scales="free")+
        scale_color_manual(values=c("blue","red"),
                name="Reward Type") +
