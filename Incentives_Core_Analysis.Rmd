...
## MODELS
mod<- multinom(variable ~ r_type + r_amount + exp_all + tot_neg_eval+
                       gender+ income+ employ,data = mr_data)

mod1<- update(mod, variable ~ r_type + r_amount + exp_all + 
                      gender+ income+ employ)

mod2<- update(mod, variable ~ r_type + r_amount + exp_all + 
                      income+ employ)

mod3<- update(mod, variable ~ r_type + exp_all + 
                      income+ employ)

mod4<- update(mod, variable ~ r_type + exp_all + income)

mod5<- update(mod, variable ~ r_type + exp_all)
...
### BROBABILITY PLOT OF THE BEST PREDICTIVE MODEL
dexp <- data.frame(r_type = rep(c("Cash", "Product"), each = 9),
  exp_all = rep(c(1:9), 2))

dexp2 <- data.frame(r_type = rep(c("Cash", "Product"), each = 9,
                length.out=90), exp_all = rep(c(1:9), 10),
                income=rep(1:5, each=18,length.out=90)) 
#dexp2[[3]]=as.factor(dexp2[[3]])

dexp2$income<-factor(dexp2$income, levels = c(1:5),
                    labels = c("< 15k", "15k-19k",
                               "20k-29k", "> 30k", "No.Ans")) 


## store the predicted probabilities for each value of ses and write
pp.exp <- cbind(dexp2, predict(mod4, newdata = dexp2, type = "probs", 
                              se = TRUE))

## calculate the mean probabilities within each level of ses
by(pp.exp[, 3:5], pp.exp$r_type, colMeans)

## melt data set to long for ggplot2
lpp <- melt(pp.exp, id.vars = c("exp_all", "r_type", "income"), value.name = "probability")
...
ggplot(lpp, aes(x = exp_all, y = probability, colour = r_type)) +
  geom_line() +
  facet_grid(variable ~ income, scales="free")
