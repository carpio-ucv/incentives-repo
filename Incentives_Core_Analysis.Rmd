...
## MODEL 1
mod <- multinom(variable ~ r_type+ r_amount+ exp_all,
           data = mr_data)
modx<-multinom(variable ~ r_type+ r_amount+ exp_all
             + tot_neg_eval, data = mr_data)

modx2<-multinom(variable ~ r_type+ exp_all, data = mr_data)
...
pp.exp <- cbind(dexp, predict(modx2, newdata = dexp, type = "probs", 
                              se = TRUE))
...
lpp <- melt(pp.exp, id.vars = c("exp_all", "r_type"), value.name = "probability")
